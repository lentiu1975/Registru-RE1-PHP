...
print("Impartire fisier SQL in bucati mici pentru phpMyAdmin...\n")

# Citeste fisierul SQL mare
with open('migrate_complete.sql', 'r', encoding='utf-8') as f:
    lines = f.readlines()
...
# Imparte in fisiere de cate 500 linii (250 INSERT-uri manifest + 250 INSERT-uri entries)
lines_per_file = 500
...
if not os.path.exists('sql_parts'):
    os.makedirs('sql_parts')

for i in range(num_files):
    start = i * lines_per_file
    end = min((i + 1) * lines_per_file, len(data_lines))

    filename = f'sql_parts/part_{i+1:02d}_of_{num_files:02d}.sql'
...
print(f"\nGATA! {num_files} fisiere create in folder 'sql_parts/'")
print("\nImporteaza fisierele in phpMyAdmin in ordine:")
print("1. part_01_of_XX.sql")
print("2. part_02_of_XX.sql")
print("... si asa mai departe")
