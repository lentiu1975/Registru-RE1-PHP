...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function performSearch(query) {
        if (!query || query.trim().length < 2) {
            searchResults.innerHTML = '<div class="text-center py-5"><i class="bi bi-search" style="font-size: 4rem; color: #ddd;"></i><p class="text-muted mt-3">Introduceți un termen de căutare pentru a vedea rezultatele</p></div>';
    ...
    function displayCurrentResult() {
        ...
        if (count > 1) {
            ...
            html += '<button onclick="window.searchApp.previousResult()" ' + (currentIndex === 0 ? 'disabled' : '') + ' class="btn btn-sm btn-primary me-2">◀ Anterior</button>';
            html += '<span class="current-position mx-2 fw-semibold">' + (currentIndex + 1) + ' / ' + count + '</span>';
            html += '<button onclick="window.searchApp.nextResult()" ' + (currentIndex === count - 1 ? 'disabled' : '') + ' class="btn btn-sm btn-primary">Urmator ▶</button>';
            html += '</div>';
        }
...
Write-Host "New app.js created!" -ForegroundColor Green
