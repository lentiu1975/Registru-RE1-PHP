...
output_file = open('migrate_full.sql', 'w', encoding='utf-8')

output_file.write("-- Migrare COMPLETA cu TOATE campurile din Django\n\n")
output_file.write("SET NAMES utf8mb4;\n")
output_file.write("SET CHARACTER SET utf8mb4;\n\n")

print("Exportare containere cu TOATE campurile...")
...
for row in cursor.fetchall():
    ...
    output_file.write(f"INSERT INTO manifest_entries (manifest_id, container_number, container_type, packages, weight, goods_description, summary_number, operation_type, permit_number, position_number, operation_request) ")
    output_file.write(f"SELECT id, {container}, {tip}, {colete}, {greutate}, {marfa}, {sumara}, {tip_op}, {numar_permis}, {numar_pozitie}, {cerere_op} FROM manifests WHERE manifest_number = {manifest_num} LIMIT 1;\n")
...
print(f"\nGATA! Fisierul 'migrate_full.sql' a fost creat!")
print("Include: packages, summary_number, operation_type, permit_number, position_number, operation_request")
